...
async function downloadHtmlAsPdf(html: string, filename: string) {
  try {
    ...
    const textBlock = (value: string, x: number, width: number, size = 9.5, colour: [number, number, number] = [51, 65, 85]) => {
    ...
    const sectionTitle = (value: string) => {
      ...
      pdf.text(clean(value).toUpperCase(), margin + 4, y + 5.9);
      y += 13;
    };
    const panel = (heading: string, body: string, accent: [number, number, number] = [2, 132, 199]) => {
      ...
      if (heading) {
        ...
        pdf.text(clean(heading), margin + 8, y);
        y += 5;
      }
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(9.3);
      pdf.setTextColor(51, 65, 85);
      lines.forEach((line: string) => { pdf.text(line, margin + 8, y); y += 4.7; });
      y = startY + height + 4;
    };
    ...
    pdf.roundedRect(margin, y, 6, 30, 3, 3, "F");
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(8);
    pdf.setTextColor(125, 211, 252);
    pdf.text("MANAGER TEAM HEALTH REPORT", margin + 11, y + 8);
    pdf.setFontSize(19);
    pdf.setTextColor(255, 255, 255);
    const titleLines = pdf.splitTextToSize(title, contentWidth - 20);
    pdf.text(titleLines, margin + 11, y + 17);
    y += 36;

    const heroScore = clean(report.querySelector(".hero .score")?.textContent);
    if (heroScore) panel("Current team health", heroScore, [126, 34, 206]);
    ...
    if (cards.length) {
      ...
      for (let index = 0; index < cards.length; index += 2) {
        ...
        pair.forEach((card, cardIndex) => {
          const x = margin + cardIndex * ((contentWidth - 5) / 2 + 5);
          const width = (contentWidth - 5) / 2;
          const label = clean(card.querySelector(".label")?.textContent) || "Metric";
          const value = clean(card.querySelector(".value")?.textContent) || clean(card.textContent);
          pdf.setFillColor(248, 251, 255);
          pdf.setDrawColor(186, 230, 253);
          pdf.roundedRect(x, y, width, 21, 3, 3, "FD");
          pdf.setFont("helvetica", "bold"); pdf.setFontSize(7.5); pdf.setTextColor(100, 116, 139);
          pdf.text(label.toUpperCase(), x + 4, y + 6);
          pdf.setFontSize(14); pdf.setTextColor(7, 89, 133);
          pdf.text(pdf.splitTextToSize(value, width - 8).slice(0, 2), x + 4, y + 14);
        });
        y += 25;
      }
    }

    for (const element of Array.from(report.children)) {
      if (element.tagName === "H2") {
        sectionTitle(clean(element.textContent));
        continue;
      }
      const className = element.className || "";
      if (typeof className !== "string") continue;
      if (className.includes("health-band")) {
        panel(clean(element.querySelector("h3")?.textContent), clean(element.textContent), [37, 99, 235]);
      } else if (className.includes("creator-scorecard")) {
        const name = clean(element.querySelector("h3")?.textContent) || "Creator";
        const score = clean(element.querySelector(".health-total")?.textContent);
        const tiles = Array.from(element.querySelectorAll(".score-tile")).map((tile) => clean(tile.textContent)).join("  •  ");
        const notes = Array.from(element.querySelectorAll(".scorecard-note")).map((note) => clean(note.textContent)).join("  ");
        panel(`${name}  ${score}`, `${tiles}  ${notes}`, [126, 34, 206]);
      } else if (className.includes("panel") || className.includes("summary-section")) {
        const heading = clean(element.querySelector("h3")?.textContent);
        const body = clean(element.textContent);
        if (body) panel(heading, body);
      }
    }
...
export default function CreatorIntelligenceDashboard({
  ...
  async function downloadYesterdayDiamondsReport() {
    ...
    const matchingAquaRows = rows
      .filter(isAquaRow)
      .filter((row) => !isHiddenAquaCreatorUsername(getUsername(row)))
      .filter((row) => {
