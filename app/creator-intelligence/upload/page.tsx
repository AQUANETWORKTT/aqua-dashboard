...
export default function DataAnalysisUploadPage() {
  ...
  const selectedMonth =
    MONTHS.find((item) => item.value === month) || MONTHS[5];
  ...
  async function loadExistingDays() {
    ...
    try {
      const res = await fetch(
        `/api/creator-intelligence/upload-status?month=${month}&t=${Date.now()}`,
        {
          cache: "no-store",
        }
      );
  ...
  async function handleImport() {
    ...
    const res = await fetch("/api/creator-intelligence/import", {
      method: "POST",
      body: formData,
      cache: "no-store",
    });
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 rounded-[2rem] border border-yellow-300/30 bg-gradient-to-br from-black via-[#111] to-[#1b1300] p-6">
          <h1 className="text-4xl font-black uppercase text-yellow-300">
            Upload Aqua Creator Stats
          </h1>

          <p className="mt-3 text-white/60">
            Select the month, add each Aqua Excel file into the correct day slot, then
