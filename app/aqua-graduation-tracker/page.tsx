...
export default function AquaGraduationTrackerPage() {
  ...
  const rowsUntilEndDay = useMemo(() => {
    return rows.filter((row) => {
      ...
      const isAquaAgency =
        String(row.agency || "").trim().toLowerCase() === "aqua";

      return dayNumber >= 1 && dayNumber <= endDay && isAquaAgency;
    });
  }, [rows, endDay]);
  ...
  }, [rowsUntilEndDay, team, search, latestGraduationUploadDay, lastDay]);
  ...
  const graduationReportRows = useMemo(() => {
    return graduationTrackerRows
      .filter((creator) => {
        ...
        return hasGraduationChance && stillNeedsToGraduate;
      })
      .sort((a, b) => {
        if (a.status !== b.status) {
          const order: Record<GraduationStatus, number> = {
  ...
  }, [graduationTrackerRows]);

  const graduationWhatsappReport = useMemo(() => {
    const header = [
      "🎓 Aqua Graduation Eligibility Report",
      "Agency: Aqua",
  ...
  function downloadGraduationWhatsappReport() {
    ...
    link.href = url;
    link.download = `aqua-graduation-report-${month}-${REPORT_MINIMUM_PROGRESS}-percent-plus.txt`;
  ...
  return (
    <main className="min-h-screen bg-[#070707] px-4 py-6 text-white">
      ...
      <div className="mx-auto max-w-[1600px]">
        <section className="mb-8 rounded-[2rem] border border-blue-400/30 bg-gradient-to-br from-black via-[#06111f] to-[#020b18] p-6 shadow-2xl shadow-blue-900/20">
          <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.35em] text-blue-300/70">
                Aqua Graduation Intelligence
              </p>
              <h1 className="mt-3 text-4xl font-black uppercase tracking-tight text-blue-300 md:text-6xl">
                Aqua Graduation Tracker
              </h1>
              <p className="mt-4 max-w-4xl text-white/60">
                Tracks Aqua creators towards 200,000 diamonds. Creators appear once they have an eligible graduation status and have reached at least 1,000 diamonds in the selected month-to-date.
              </p>
            </div>
            ...
            <div className="flex shrink-0 flex-col gap-3 sm:flex-row md:flex-col">
              ...
              <Link
                href="/aqua-dashboard"
                className="rounded-xl border border-blue-300/30 bg-blue-400/10 px-6 py-3 text-center font-black uppercase text-blue-300 transition hover:scale-[1.02] hover:bg-blue-400/20"
              >
                🐬 Aqua Dashboard
              </Link>
        ...
        <section className="mb-6 grid gap-4 rounded-3xl border border-white/10 bg-white/[0.03] p-4 md:grid-cols-4">
          <div>
            ...
            <select
              ...
              className="mt-2 w-full rounded-xl border border-blue-300/20 bg-black px-3 py-3 text-white"
            >
              {MONTHS.map((item) => (
          ...
          <div>
            ...
            <input
              ...
              className="mt-2 w-full rounded-xl border border-blue-300/20 bg-black px-3 py-3 text-white outline-none"
            />
          </div>
        </section>

        {loading && (
          <div className="mb-6 rounded-2xl border border-blue-300/20 bg-blue-300/10 px-4 py-3 text-green-100">
            Loading Aqua graduation data...
          </div>
        )}

        <section className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-3xl border border-blue-300/20 bg-black/60 p-5">
            <p className="text-xs font-black uppercase text-white/40">Aqua Tracker Creators</p>
            <p className="mt-3 text-3xl font-black text-blue-300">{formatNumber(graduationTrackerRows.length)}</p>
          </div>
        ...
        <section className="mb-6 rounded-3xl border border-blue-400/30 bg-blue-500/10 p-6">
          <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
            <div>
              <h2 className="text-2xl font-black uppercase text-blue-300">Graduation Eligibility Tracker</h2>
            ...
            <div className="rounded-2xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white/60">
              Target pace by latest upload day {latestGraduationUploadDay}: <strong className="text-blue-200">{formatNumber((GRADUATION_TARGET / lastDay) * Math.min(latestGraduationUploadDay, lastDay))}</strong>
            </div>
          </div>

          <div className="mb-4 rounded-2xl border border-blue-400/20 bg-black/40 p-4">
            <div className="mb-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <h3 className="text-lg font-black uppercase text-blue-200">WhatsApp Graduation Report</h3>
                <p className="mt-1 text-sm text-white/45">
                  Aqua-only report. Only creators at {REPORT_MINIMUM_PROGRESS}%+ progress and under 200k diamonds are included.
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-end gap-2">
                ...
                <button
                  ...
                  className="rounded-xl bg-blue-400 px-4 py-3 text-xs font-black uppercase text-blue-950 transition hover:scale-[1.02]"
                >
                  Copy WhatsApp Text
                </button>
                ...
                <button
                  ...
                  className="rounded-xl border border-blue-300/40 bg-blue-300/10 px-4 py-3 text-xs font-black uppercase text-blue-200 transition hover:bg-blue-300/20"
                >
                  Download Report
                </button>
              </div>
                ...
                {graduationTrackerRows.length ? (
                  graduationTrackerRows.map((creator) => (
                    <tr key={creator.username} className="border-t border-white/10">
                      ...
                      <td className="p-3 text-blue-200">{formatNumber(creator.diamonds)}</td>
                      <td className="p-3">
                  ...
                  <tr className="border-t border-white/10">
                    <td className="p-3 text-white/50" colSpan={11}>
                      No eligible Aqua graduation creators found for this month/filter. Check that graduation_status is imported and creators have reached at least 1,000 diamonds.
                    </td>
                  </tr>
                )}
              </tbody>
