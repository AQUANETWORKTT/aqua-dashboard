...
function getSupabaseAdmin() {
  ...
  console.log("SERVICE ROLE START:", serviceRoleKey?.slice(0, 20));
...
function parseHours(value: any): number {
  ...
  if (str.includes(":")) {
    ...
    if (parts.length === 3) return Number((parts[0] + parts[1] / 60 + parts[2] / 3600).toFixed(2));
    if (parts.length === 2) return Number((parts[0] + parts[1] / 60).toFixed(2));
  }
...
function getCreatorStatus(row: Record<string, any>) {
  ...
  const validDays = parseNumber(getValue(row, ["Valid go LIVE days", "Valid live days"]));
  ...
  if (daysJoined <= 14 && diamonds >= 10000 && hours >= 10 && followers >= 100) return "High Potential";
...
function parseCreatorStatsFile(buffer: Buffer, statsDate: string) {
  ...
  for (const row of rows) {
    ...
    parsedRows.push({
      ...
      username: cleanUsername(getValue(row, ["Creator's username", "Creators username", "Creator username", "Username"])),
      manager: String(getValue(row, ["Creator Network manager", "Creator network manager", "Manager"]) ?? "").trim(),
      ...
      live_duration_hours: parseHours(getValue(row, ["LIVE duration", "Live duration", "Duration"])),
      valid_go_live_days: parseNumber(getValue(row, ["Valid go LIVE days", "Valid live days"])),
